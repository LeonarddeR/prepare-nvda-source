...
- A **Windows** runner with Visual Studio 2022 Enterprise installed (e.g. `windows-2022`). The
  action runs the VS installer against NVDA's `.vsconfig` to add the components `scons source`
  needs to compile NVDA's native code.
...
`SCONS_CACHE_MSVC_CONFIG`), keyed by the runner OS, image version, and a hash of NVDA's
